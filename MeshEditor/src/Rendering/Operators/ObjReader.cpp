...
Rendering::Model::Mesh* ObjReader::LoadMesh(const std::string &path)
{
	vector<glm::vec3> vertices;
	vector<vector<unsigned int>> faces;
	Operators::ObjReader::ReadObj(path, vertices, faces);
	auto newMesh = new Rendering::Model::Mesh();
	Operators::MeshConverter::ArrayToHalfEdgeStructure(*newMesh, vertices, faces);

	return newMesh;
}

void ObjReader::SaveMesh(const Rendering::Model::Mesh& mesh, const std::string &path)
{
